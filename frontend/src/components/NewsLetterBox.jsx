...
const NewsLetterBox = () => {
  ...
  return (
    <div className='text-center'>
        ...
        <form onSubmit={onSubmitHandler} className='w-3/4 min-sm:w-3/5 flex items-center gap-3 mx-auto my-6 border pl-3 bg-white'>
            <input type='email' placeholder='Enter your email' className='w-3/4 sm:flex-1 outline-none max-sm:text-[0.6rem]'  required/>
            <button type='submit' className='bg-[#00b3ff] font-medium cursor-pointer hover:bg-[#006eff] text-white px-4 py-2 sm:ml-2 max-sm:text-[0.65rem]'>Subscribe</button>
        </form>
    </div>
  )
}
